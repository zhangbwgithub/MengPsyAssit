...
## 当前状态：S1 进行中——T-S1.1~S1.18 十七卡连发 ✅ 合 main（2026-08-27~30），批次 A 来访者档案（API+前端）全部上线，反馈台账运行中（FB-001~014）

### 进行中
- （无进行中任务卡；批次 A 收官，下一批=批次 B：多途径输入/私密笔记/导出四象限，待陛下发话）

### 已完成（S1 增补·四）
- **T-S1.18 来访者档案前端** ✅（merge 0870ea0）：批次 A 前端卡。左列「来访」区（分组之上）：姓名+状态徽标（进行中绿/已结束灰）+小节数、点击过滤记录（66→2 行实测）+「来访者·X 清除」提示条；新增/编辑/删除弹层（姓名必填/性别单选/年龄/电话/紧急联系人×2/开始时间/状态/备注，422 显错）；删除确认文案「名下 N 条记录将保留并归为未关联」；记录条目来访徽标+内联归属下拉；上传区来访者下拉（上传后自动补挂）；详情区来访者字段；Workbench KPI/待办/主题切换 emoji 全清改单色 SVG（atlantic/idh 基准第一步）。验收：86 pytest + ruff + build 全过；真浏览器沙箱实测 **11/11 全过零 JS 异常**（首轮 9/11 两项系验收脚本选择器问题，补测 3/3 证实产品无恙）；线上 5199 冒烟全过（来访区/新增按钮/无 emoji/双 Tab 齐全）。
- **T-S1.17 来访者档案 API** ✅（merge bfe40a1）：FB-014 客户需求①，批次 A 后端卡。Client 表加 8 列（name/gender/age/phone/紧急联系人×2/start_date/session_count_manual，列自愈）；clients CRUD（重名/空名/非法日期/非法 status 均 422，code 自动回填）；`PATCH /sessions` 挂/解绑 `client_id`（不存在 404）；列表/详情/导出富化 `client_id/client_name`；小节数口径（手工值优先，否则自动计会话数）。验收：86 pytest（+4）+ ruff 全过；路由前缀警告生效注册正确；真 API 沙箱实测 13/13 全过。
- **FB-013 工作台**：陛下参考稿「朗庭心空间」拍板先做看板、实时录音延后；**T-S1.16 工作台页** ✅（merge 9029016）：页头「记录看板/工作台」胶囊切换；Workbench.vue 883 行纯 CSS/SVG：4 KPI 卡（周环比 ↑↓◆）+统计窗口、2×2 图表（donut/词云/14天趋势/7天条形）、待办三卡下钻。真浏览器 7/7 全过零异常。**T-S1.15 统计 API** ✅（merge 30f555a）：`GET /dashboard/summary` 单端点（周 KPI+环比/状态分布/词云 Top20/14天趋势补 0/待办三项），82 pytest 全过；又踩 /api 前缀坑，大统领修复（581344c）。
- **FB-014**：客户新需求（MengPRD.md）解读 + UI 风格基准拍板（atlantic.vc + idh_dashboard：SF 字体/低饱和色板/大圆角/禁 emoji）+ 陛下裁决录音延后，入 `docs/feedback.md`；`/home/houmo/meng/MengPanel.md` 升级为客户需求基线（批次 A/B/录音闭环/体验增强分层）。
